...
- `Current stream`: movement since the current Fansly live stream started. This is the default for new installs. The app checks stream status every few minutes with the captured Fansly auth. If the account is offline, the badge shows `Offline`.

## Overlay Customization

Use the `Overlay customization` panel on the dashboard to change the overlay title, appearance mode, width, Pop in style, the two background gradient colors, and the animated swipe color. Appearance modes include Classic, Pill, Neon, Logo, Compact, and Pop in. Logo styles include a subtle blurred background rain of small Fansly logos. Pop in stays hidden until rank movement happens, then slides in from the left or right, stays for about 3 minutes, and slides back out; its style can be Default, Pill, or Logo. Rank numbers turn gold automatically when the displayed rank is in the top 500. These settings are saved locally under `data/`, so they survive app restarts without being included in the repository.
